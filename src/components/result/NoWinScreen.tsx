...
const staggerItem = (delay: number, shouldReduce: boolean) => ({
  initial: shouldReduce ? false : { opacity: 0, y: 10 },
  animate: { opacity: 1, y: 0 },
  transition: { duration: 0.26, delay, ease: [0.23, 1, 0.32, 1] as [number,number,number,number] },
});
...
export default function NoWinScreen({ name }: Props) {
  ...
  return (
    ...
    >
      ...
      <motion.div {...staggerItem(0.05, shouldReduce ?? false)}>
        <div
        ...
        >
          <svg viewBox="0 0 48 48" width="36" height="36" fill="none" aria-hidden>
            <rect x="12" y="16" width="24" height="24" rx="4" fill="#1E5631" opacity="0.25" />
            <rect x="15" y="19" width="18" height="18" rx="3" fill="#1E5631" opacity="0.4" />
            <rect x="14" y="10" width="20" height="9" rx="3" fill="#F5A623" opacity="0.6" />
            <rect x="17" y="13" width="14" height="3" rx="1.5" fill="#1E5631" opacity="0.5" />
            <text x="24" y="32" textAnchor="middle" fontSize="7" fontFamily="Georgia,serif" fontWeight="bold" fill="#F5A623" opacity="0.7">BM</text>
          </svg>
        </div>
      </motion.div>
      ...
      <div className="flex flex-col gap-3 max-w-xs">
        <motion.h1
          {...staggerItem(0.1, shouldReduce ?? false)}
          className="font-display font-bold text-white"
          style={{ fontSize: 'clamp(1.8rem, 6vw, 2.4rem)', letterSpacing: '-0.02em', textWrap: 'balance' }}
        >
          Thank You, {name}.
        </motion.h1>

        <motion.p
          {...staggerItem(0.22, shouldReduce ?? false)}
        ...
        <motion.div
          {...staggerItem(0.36, shouldReduce ?? false)}
        ...
        >
          <p className="font-body text-xs font-semibold tracking-widest uppercase mb-1.5" style={{ color: 'rgba(255,255,255,0.35)' }}>
            Next draw
          </p>
          <p className="font-body text-sm leading-relaxed" style={{ color: 'rgba(255,255,255,0.7)' }}>
            ...
            <span className="font-semibold" style={{ color: '#4CAF72' }}>
              {formatDate(nextDraw)}.
            </span>
          </p>
        </motion.div>
      ...
      {/* CTAs */}
      <motion.div
        {...staggerItem(0.52, shouldReduce ?? false)}
      ...
      <motion.a
        {...staggerItem(0.65, shouldReduce ?? false)}
        href="/"
